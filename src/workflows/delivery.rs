...
use crate::company::owners::owner_recipients;
...
use crate::ports::{
    ...
    EventLog, InboxStore, UserStore, generate_id, now_millis,
};
...
// `owner_recipients` now lives in `crate::company::owners` (imported above) so
// the parked-approval notification path (issue #750, `runtime::cycle`) resolves
// `owner` recipients through the same server-side rule as workflow delivery,
// rather than growing a second resolver that could drift from this one's
// security boundary. `workflows` is `openhuman`-gated; the resolver is not.
...
mod tests {
    ...
    use crate::ports::{UserRecord, UserRole, UserStatus};
