...
The routes that write a secret are split again into
[`api-write-plane-credentials.md`](api-write-plane-credentials.md).
...
Moved to
[api-write-plane-credentials.md](api-write-plane-credentials.md#credential-bearing-surfaces-feature-gated)
— the credential, domain and SMTP routes, the password-as-patch rule, and the
per-company serialization the legacy-blob migration needs.

### Retired native OAuth callback

Moved to
[api-write-plane-credentials.md](api-write-plane-credentials.md#retired-native-oauth-callback).
