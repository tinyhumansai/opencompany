...
cargo run --bin opencompany -- serve --company examples/agentic_marketing_agency

# 2. From examples/console — the console on http://localhost:5173
npm install
npm run dev
```
...
> This is a Vite/TypeScript app, not a Cargo package — it is excluded from the
> workspace in the root `Cargo.toml`, so `cargo build --workspace` ignores it.
