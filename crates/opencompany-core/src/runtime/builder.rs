...
impl RuntimeBuilder {
    ...
    pub async fn build(mut self) -> Result<CompanyRuntime> {
        ...
        if let (Some(current), Some(previous)) = (
            self.search_backend.take(),
            handover
                .as_ref()
                .and_then(|handover| handover.search_backend.as_ref()),
        ) {
            self.search_backend = Some(current.with_ledger_from(previous));
        }
...
mod tests_part3;
#[cfg(test)]
#[path = "builder_tests_part4.rs"]
mod tests_part4;
#[cfg(test)]
#[path = "builder_tests_part5.rs"]
mod tests_part5;
#[cfg(test)]
